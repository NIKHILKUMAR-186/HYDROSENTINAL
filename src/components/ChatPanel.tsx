...
const SUGGESTIONS = [
  { text: "Is this water safe to drink?", icon: "💧", desc: "Safety check" },
  { text: "Analyze current readings", icon: "📊", desc: "Metrics" },
  { text: "Predict water quality", icon: "🔮", desc: "Forecast" },
  { text: "Suggest actions", icon: "✅", desc: "Guidance" },
];
...
export const ChatPanel = () => {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: "assistant", content: "👋 Hi! I'm your AI water quality expert. Ask me anything about your readings, safety analysis, or personalized guidance." },
  ]);

  const isRateLimitMessage = (value: string) => /rate limit|too many requests|429/i.test(value);

  const getAskErrorMessage = (error: unknown, data: unknown) => {
    const responseData = data && typeof data === "object" ? (data as { error?: unknown; retryAfterSeconds?: unknown }) : null;
    const dataMessage = typeof responseData?.error === "string" ? responseData.error : "";
    ...
    if (dataMessage && isRateLimitMessage(dataMessage)) {
      return Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
        ? `Rate limit reached. Please try again in ${retryAfterSeconds} seconds.`
        : "Rate limit reached. Please try again shortly.";
    }

    if (errorMessage && isRateLimitMessage(errorMessage)) {
      return Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
        ? `Rate limit reached. Please try again in ${retryAfterSeconds} seconds.`
        : "Rate limit reached. Please try again shortly.";
    }

    return dataMessage || errorMessage || "Failed to get answer";
  };
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (error || data?.error) {
        throw new Error(getAskErrorMessage(error, data));
      }
